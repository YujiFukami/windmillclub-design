...
| 元MATLABのファイル | VBA側の対応 | 出力設定キー |
|---|---|---|
| `output_FEM_analyze_data.csv` | `Mod21_桁結果CSV出力.WriteSparSingleCsv` | `構造_単一` |
| `output_BMDとか.csv` | `WriteSparBmdCsv` | `構造_BMD可視化` |
| `output_基本積層応力.csv` | `WriteSparPlyStressCsv` | `構造_基本積層応力` |
| `output_基本積層歪.csv` | `WriteSparPlyStrainCsv` | `構造_基本積層歪` |
| `output_部分積層応力.csv` | `WriteSparUdStressCsv` | `構造_部分積層応力` |
| `output_部分積層歪.csv` | `WriteSparUdStrainCsv` | `構造_部分積層歪` |
| `output_ketasiken_date.csv` | `WriteSparTestCsv` | `構造_桁試験照合` |
| `keta,aoa=X.csv`(新規) | `Mod22_桁構造解析スイープ実行.RunSparSolveSweep` | `構造_スイープ詳細` |
...
- **ヘッダー/データの列ずれ**: 元コードの`output_FEM_analyze_data.csv`はヘッダー行とデータ列の対応にずれがある(ソース中に重複したカンマがある)既知の問題がありましたが、ここでは正しく対応した列見出しを出力します。
...
出力するファイルの取捨選択は、空力側と共通の「設定」シートの「出力ファイル設定」テーブルで行います(`Mod19_出力共通.ShouldOutput`、テーブル未作成時は既定で「出力する」にフォールバック)。
