...
にそのまま対応します(変数名は`u1`〜`u5`、`q11`〜`q66`と理論編の記号を踏襲しています)。理論編§3のA行列(厚み比重み付き平均)は、
...
桁位置(KetaIti)は空力側シートがパーセント表記(例: 35=35%)で保持しているため、`/ 100#`で分率化してから空力中心Cp(0〜1)と揃えて使います。この単位を揃え忘れるとねじりモーメントが2桁ずれて暴走する、という実機テストで見つかった不具合があった箇所です(詳細は`07_既知の制限事項.md`)。
...
理論編§8のTsai-Wu複合則は`TsaiWuRatio`(2次方程式`a・R²+b・R=1`を解く部分)、材料主軸への応力回転は`RotateToMaterialAxes`が担当します。`EvaluateSparStrength`本体では、FEMで求めた断面内力・変位差(ひずみ)から、断面周方向の各点・各層の応力を計算し、
